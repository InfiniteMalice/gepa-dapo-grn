...
class DAPOTrainer:
    ...
    def train_step(
        ...
        logp_new = _log_probs_from_logits(outputs.logits, batch.actions)
        with torch.no_grad():
            ref_outputs = self.ref_policy(**batch.inputs)
            logp_ref = _log_probs_from_logits(ref_outputs.logits, batch.actions)
    ...
    def state_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ref_policy": self.ref_policy.state_dict() if self.ref_policy else None,
        }
    ...
    def load_state_dict(self, state: Dict[str, Any]) -> None:
        ...
        ref_state = state.get("ref_policy")
        if ref_state is not None:
            if self.ref_policy is None:
                self.ref_policy = self.policy.clone()
            self.ref_policy.load_state_dict(ref_state)
